...
### 2026-05-26 商品页 Get 按钮无效果时改走直接 checkout 链接

- 现象：
  - 登录验证码最终成功，认证也完成了 `Epic store session verification success`。
  - 进入 `Down in Bermuda` 和 `Tomb Raider I-III Remastered` 商品页后，`purchase-cta-button` 文本为 `Get`，且按钮没有 disabled / aria-disabled。
  - 标准点击、事件派发、DOM click、坐标点击和 force click 都没有产生购物车、checkout iframe、已入库状态或安全校验，最终仍报 `Failed to confirm claim flow for promotions`。
- 根因判断：
  - 本周 Epic 商品页 `Get` 按钮存在“可见但点击不触发领取流程”的情况。
  - 仅靠商品页按钮进入 checkout 已不稳定；Epic 仍支持基于 `namespace` 和 `offerId` 的 `/purchase?offers=1-{namespace}-{offerId}` 直接 checkout 链接。
- 改动文件：
  - `app/services/epic_games_service.py`
  - `docs/maintenance-log.md`
- 处理结果：
